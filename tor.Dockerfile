...
# (docker-managed, root-owned-by-default) HiddenServiceDir volume -- Tor
# itself refuses to fix that ownership and hard-fails instead (unlike
# DataDirectory, which it does fix) -- before Tor drops to debian-tor via
# the 'User' directive in torrc.
